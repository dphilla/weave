...
/**
 * Adapt a Node TCP socket to an exact-read byte-stream interface.
 *
 * Reads are buffered with configurable pause, resume, and hard-cap thresholds.
 * Read and write timeouts are terminal: the socket is destroyed and all later
 * operations observe the same failure.
 */
export class TcpTransport {
  constructor(socket, options = {}) {
    ...
    if (this.resumeBytes > this.pauseBytes || this.pauseBytes > this.maxBufferedBytes) {
      throw new RangeError(
        "TCP buffer thresholds must satisfy resumeBytes <= pauseBytes <= maxBufferedBytes",
      );
    }
  ...
  /** Read exactly `size` bytes, coalescing or splitting socket chunks. */
  readExact(size) {
    if (!Number.isSafeInteger(size) || size < 0) {
      return Promise.reject(
        new RangeError("readExact size must be a non-negative safe integer"),
      );
    }
    ...
    if (size === 0) return Promise.resolve(new Uint8Array(0));
  ...
  /** Write one byte sequence, resolving after Node reports it flushed. */
  write(bytes) {
    return new Promise((resolve, reject) => {
      if (this.err) {
        reject(this.err);
  ...
  /** Destroy the underlying socket and abandon any unfinished I/O. */
  close() {
    // Destroy rather than half-close: a timed-out read must not keep a process
    // alive indefinitely while waiting for an unresponsive peer's FIN.
    try { this.socket.destroy(); } catch { /* already closed */ }
  }
}

/**
 * Peek at one socket byte without consuming it.
 *
 * This is useful for dispatching a newly accepted connection by a leading
 * discriminator. The byte is restored with `unshift()` before this promise
 * resolves, so the eventual consumer still receives the complete stream.
 */
...
/** Dial a TCP endpoint with a finite deadline and return a bounded transport. */
export function connectTcp(host, port, options = {}) {
  ...
  const transportOptions = {
    ...
    pauseBytes: options.pauseBytes,
    resumeBytes: options.resumeBytes,
  ...
  return new Promise((resolve, reject) => {
    ...
    const onConnect = () => {
      ...
      cleanup();
      try {
        resolve(new TcpTransport(socket, transportOptions));
      } catch (error) {
